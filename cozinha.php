...
if(isset($_GET['deleta']) && $_GET['deleta'] == "deleta"){ 



	$del = mysqli_query($db,"DELETE FROM cozinha WHERE qtd='1' OR produto =''") or die(mysql_error());
...
<table width="100%" border="0" class="inputs">
 ...
 $observacao = "";
	...
	$grava = mysqli_query($db,"INSERT INTO cozinha (produto, qtd, observacao)VALUES('$iten','1','$observacao')") or die($db->error);
...
<table width="100%">
	...
	$mostra = mysqli_query($db,"SELECT produto, SUM(qtd) AS quantidade FROM cozinha GROUP BY produto ")or die($db->error);

	while($ver = $mostra->fetch_assoc()){
...
<table width="100%">
    ...
    if(isset($_GET['deleta']) && $_GET['deleta'] == "deleta"){ 
	  ...
	  $del = mysqli_query($db,"TRUNCATE TABLE cozinha") or die($db->error);
