...
if(isset($_GET['adiciona'])){
  ...
  $theValue = function_exists("real_escape_string") ? $db->real_escape_string($theValue) : $db->escape_string($theValue);
...
if(isset($_POST['ok'])){
		...
		$altera = mysqli_query($db,"UPDATE pedido SET idGarcon = '$idGarcon', situacao = '1' WHERE id_mesa = '$numero' ") or die($db->error);
		if($altera){
...
function Cozinha(pagina,nome,w,h,scroll){
	...
	$carrinho = mysqli_query($db,"SELECT * FROM entrega WHERE id_mesa='$numero' AND situacao='1'") or die($db->error);
