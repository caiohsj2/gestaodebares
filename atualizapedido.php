...
date_default_timezone_set('America/Sao_Paulo');
 ...
 if(isset($_GET['btn']) && $_GET['btn'] =="cancela"){

	 	$mesa = $_GET['id_mesa'];

		$num = $_GET['numero'];

		$del = mysqli_query($db,"DELETE FROM entrega WHERE id_mesa = '$mesa' ") or die(mysql_error());
...
<table width="100%" border="0" class="inputs">
  ...
  $sql = mysqli_query($db,"SELECT * FROM entrega WHERE id_mesa = '$mesa'") or die(mysql_error());

  $cont = $sql->fetch_assoc();

  $id_mesa = $cont['id_mesa'];

  

  $n = mysqli_query($db,"SELECT * FROM config");
